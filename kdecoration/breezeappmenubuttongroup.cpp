    ...
    , m_opacity(1)
{
    ...
    connect(decoratedClient, &KDecoration2::DecoratedClient::hasApplicationMenuChanged, this, &AppMenuButtonGroup::updateAppMenuModel);
...
#if CUSTOM_DECORATION_DEBUG_MODE
        qDebug() << this << "setCurrentIndex" << m_currentIndex;
#endif
...
void AppMenuButtonGroup::setOverflowing(bool set)
{
    if (m_overflowing != set) {
        m_overflowing = set;
#if CUSTOM_DECORATION_DEBUG_MODE
        qDebug() << this << "setOverflowing" << m_overflowing;
#endif
...
void AppMenuButtonGroup::setHovered(bool value)
{
    if (m_hovered != value) {
        m_hovered = value;
#if CUSTOM_DECORATION_DEBUG_MODE
        qDebug() << this << "setHovered" << m_hovered;
#endif
...
void AppMenuButtonGroup::setShowing(bool value)
{
    if (m_showing != value) {
        m_showing = value;
#if CUSTOM_DECORATION_DEBUG_MODE
        qDebug() << this << "setShowing" << m_showing << "alwaysShow" << m_alwaysShow << "currentIndex" << m_currentIndex << "opacity" << m_opacity;
#endif
...
void AppMenuButtonGroup::setAlwaysShow(bool value)
{
    if (m_alwaysShow != value) {
        m_alwaysShow = value;
#if CUSTOM_DECORATION_DEBUG_MODE
        qDebug() << this << "setAlwaysShow" << m_alwaysShow;
#endif
...
#if CUSTOM_DECORATION_DEBUG_MODE
    qDebug() << "AppMenuModel" << m_appMenuModel;
#endif
}
...
void AppMenuButtonGroup::updateAppMenuModel()
{
    ...
    if (m_appMenuModel) {
        ...
        // Init AppMenuModel
#if CUSTOM_DECORATION_DEBUG_MODE
        qDebug() << "windowId" << decoratedClient->windowId();
#endif
...
void AppMenuButtonGroup::trigger(int buttonIndex)
{
    ...
    if (actionMenu && deco) {
        ...
        rootPosition.ry() = deco->windowPos().y() - 2 * deco->settings()->smallSpacing();
...
QAction *AppMenuButtonGroup::findActionUnderCursor(QPoint pt)
{
    ...
    pt.ry() -= (d->titleBar().height() + 2 * d->settings()->smallSpacing());
    ...
    while (nestedMenu && !actionAtPt) {
        ...
        if (!actionAtPt) {
            if (nestedMenu->activeAction()) {
                ...
                if (nestedMenu) {
                    pt.ry() -= nestedMenu->y() - d->windowPos().y() + 2 * d->settings()->smallSpacing();
                }
            } else {
                nestedMenu = nullptr;
            }
