...
void TextButton::setText(const QString &newText)
{
    ...
    auto geom = geometry();
    geom.setSize({textSize.width() + (2 * d->settings()->smallSpacing() * 2), textSize.height()});
    setGeometry(geom);
...
void TextButton::paint(QPainter *painter, const QRect &repaintArea)
{
    ...
    painter->drawText(geometry(), Qt::TextSingleLine | Qt::AlignCenter | Qt::TextHideMnemonic, m_text);
    painter->restore();
}
